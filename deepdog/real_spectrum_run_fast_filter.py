...
def get_a_result_fast_filter(input) -> int:
	...
	for di in dot_inputs:
		vals = pdme.util.fast_v_calc.fast_vs_for_dipoleses(numpy.array([di]), current_sample)
		current_sample = current_sample[
			pdme.util.fast_v_calc.between(vals, lows, highs)
		]
	return len(current_sample)


class FastFilterCompareRun:
	"""
		...
		for model_count, model in enumerate(self.models):
			...
			with multiprocessing.Pool(core_count) as pool:
				...
				while (cycles < self.max_monte_carlo_cycles_steps) and (
					...
					cycle_success_ff += current_success_ff
					_logger.debug(f"current running successes: {cycle_success}")
					_logger.debug(f"current running successes: {cycle_success_ff}")
